...
async function createCheckount(carrello, cliente_id) {
  ...
  try {
    ...
    for (const update of productsToUpdate) {
      const prodotto = (await getProduct(update.id, client)).product;
      //console.log("prodotto", prodotto.product);
      const result = await updateProduct(update.id, prodotto.nome, prodotto.descrizione, prodotto.prezzo, update.newAvailability, client);
      if (!result.success) {
        await rollbackTransaction(client);
        return {
          success: false,
